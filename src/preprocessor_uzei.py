...
class UzeiCleaner:
    def __init__(self):
        self.namespaces = {'basic': 'http://www.tbxinfo.net/ns/basic',
              'min': 'http://www.tbxinfo.net/ns/min',
              'tbx': 'urn:iso:std:iso:30042:ed-2',
              'teka': 'https://uzei.eus/tbx/ns/teka',
              'xml': 'http://www.w3.org/XML/1998/namespace'
              }
        current_folder = Path(__file__).parent
        config_path = Path(current_folder, 'valores_uzei.json')
    ...
    def extract_termData(self, langSections, conceptNode, lang_registry):
        ...
        for langSec in langSections:
            ...
            for tbxTermSec in tbxTermSecs:
                ...
                tbxPOS = tbxTermSec.find('./teka:termLexicalCategory', self.namespaces)
                grmr_config = self.config['categoria_gramatical']
                if tbxPOS == None:
                    posValue = ''
                    numberValue = ''
                else:
                    if tbxPOS.text in grmr_config.keys():
                        posValue = grmr_config[tbxPOS.text]['pos']
                        numberValue = grmr_config[tbxPOS.text]['number']
                    else: 
                        posValue = ''
                        numberValue = ''
                        print(f'Warning: {tbxPOS.text} value was lost')
                # create a base id for the term to be used in the entryID and formID
                cleanTerm = string_cleaner(term)
                if posValue != '':
                    termID = f"{conceptNode.attrib['conceptID']}_{language}_{cleanTerm}_{posValue}"
                else:
                ...
                cleanEntryNode.set('pos', posValue)
                ...
                # prepare formID
                if numberValue != '':
                    formID = f"{termID}_{numberValue}_form"
                else:
                ...
                numberNode = ET.SubElement(cleanFormNode, 'number')
                numberNode.text = numberValue
        return conceptNode
